...
from stack_constructs.cognito import Cognito
import traceback
...
class BedrockAPIStack(Stack):
    ...
    def build_full(self):
        ...
        bedrock_cost_tracking_manual = lambda_function.build(
            function_name=f"{self.prefix_id}_bedrock_cost_tracking_manual",
            code_dir=f"{self.lambdas_directory}/cost_tracking_manual",
            memory=512,
            timeout=900,
            environment={
                "LOG_GROUP_API": f"/aws/lambda/{self.prefix_id}_bedrock_invoke_model",
        ...
        # ==================================================
        # ============= LAMBDA DDB RETRIEVAL ===============
        # ==================================================

        bedrock_ddb_cost_retrieval = lambda_function.build(
            function_name=f"{self.prefix_id}_bedrock_ddb_cost_retrieval",
            code_dir=f"{self.lambdas_directory}/ddb_cost_retrieval",
            memory=512,
            timeout=900,
            environment={
                "BEDROCK_URL": self.bedrock_runtime_endpoint_url,
        ...
        # =============== Cognito User Pool ================
        # ==================================================
        cognito_init = Cognito(self,id=f"{self.prefix_id}_cognito-user-pool")
        cognito_user_pool = cognito_init.build()
        ...
        Auth = apigw.CfnAuthorizer(self, "Auth", rest_api_id=api_gw.rest_api_id,
        ...
        api_cost_track_manual = api_route.build(
            lambda_function=bedrock_cost_tracking_manual,
            route="cost_track_manual",
            method="POST",
            auth=Auth,
            validator=False,
        )

        api_ddb_cost_retrieval = api_route.build(
            lambda_function=bedrock_ddb_cost_retrieval,
            route="ddb_cost_retrieval",
            method="POST",
            auth=Auth,
            validator=False
        )
        ...
        api_key_class = APIKey(
            ...
            dependencies=[api_gw, api_invoke, api_cost_track_manual, api_ddb_cost_retrieval]
        )
